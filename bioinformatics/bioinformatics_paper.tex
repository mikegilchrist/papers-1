...
\pkg{\package} is a fast and reliable R package for estimating biological parameters, such as selection against translation inefficiency, nonsense error rates, and ribosome pausing times, for sets of genes based on codon or amino acid scale data, such as ribosome foot print or synonymous codon frequency.
...
Finally, \package also comes with a set of visualization routines and the ability to revisit or reinitiate previous model fittings, providing researchers with a well rounded easy to use framework.
