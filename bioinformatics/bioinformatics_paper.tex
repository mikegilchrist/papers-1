...
\pkg{\package} is an R package for estimating biological parameters, such as selection against translation inefficiency, nonsense error rate, and ribosome pausing time, for sets of genes based on codon or amino acid data.
...
Current models implemented in \package can accurately estimate relevant parameters given either coding sequences or ribosome foot-printing data.
...
Here, we describe an open-source software implemented in R \citep{rcore} that allows researchers to analyze genome-scale data like coding sequences and ribosome foot-printing data using a Bayesian framework. Models described by \citet{gilchrist2015}, \citet{wallace2013}, and \citet{shah2011} can be effectively fitted using \packagee. 
...
The ROC model extends the codon usage bias model developed by \citet{gilchrist2015,wallace2013,shah2011}, which can reliably estimate the strength of selection on \underline{r}ibosome \underline{o}verhead \underline{c}ost (ROC), mutation bias, and allows for the inference of protein synthesis rates.
...
Aside from just identifying the most efficient codons, ROC provides information on the direction of mutation bias, which can be used to approximate mutation rates between codons (see \citep{gilchrist2015,wallace2013}).
The ability to estimate protein synthesis rates in the absence of empirical data is useful for investigating CUB of non-model organisms where such data is lacking.
Use of the mixture model allows for the investigation of codon usage bias heterogeneity at the genome or gene level.  

Additional models included in \package provide estimates of nonsense errors rates for each codon (FONSE) and pausing time estimates from ribosome footprinting data (PA). 
\package also implements a ribosome \underline{Pa}using (PA) model to estimate codon specific ribosome pausing times from ribosome foot-printing data.
These models can provide an expectation for experimental work.
Significant deviations from the estimated parameters might suggest an underlying assumption of the model is violated or the selective forces involved are particularly weak for an organism.
For example, if the estimated protein syntheis rates from ROC do not correlate with empirical data, then selection for translation efficiency migth be relatively weak in an organism.
These types of observations can serve as the basis for future empirical and computational studies.

The underlying object-oriented framework allows users to extend the current package to include their own models for biological data analysis.
