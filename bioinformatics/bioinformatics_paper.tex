...
%As all implemented model contain gene specific parameters (e.g. protein production rate) in addition to set specific parameters (e.g. mutation) we extended the mixture approach implemented in \package to incorporate the need for a gene specific parameter. 
...
%\citet{AUTHORYEAR} is 'cite in text' and appears as '... AUTHOR (YEAR)'
%\citep{AUTHORYEAR} is 'cite in parentheses and appears as '... (AUTHOR, YEAR)'
...
%The PA model assumes that ribosome foot-print counts are inverse proportional to pausing time allowing for the estimation of the distribution of ribosome pausing times for each codon. 
...
\package provides an interface to connect a model with the implemented MCMC algorithm. 
