...
% Use popgen models is better
% - break down mutation and selection
% - use this information to calculate selection, genetic load, ...
% - Our stuff is orthogonal to synteny
% - we can think about the introgression by calculating genetic load
% - Huge genetic load means that something advantageous must be in that region
% - selection would not allow this introgression to happen based on this genetic load
% - Alternative way of aging the introression
% - model of mutation decay to age introgression
% - now caveats 
% - if we fail to account for stuff we get screwed up results
% - if we don't have previous indication of region or assignment we may miss it.
% - tAI, CAI can not do this
