...
Recognizing the two different signatures of mutation bias and selection improves our ability to predict protein synthesis rate by $42 \%$ and allowed us to accurately assess codon preferences.
...
AIC values strongly support the hypothesis that the \kluyveri genome consists of genes with two different and distinct patterns of codon usage bias rather than a single ($\Delta\text{AIC} = 75,462$; Table \ref{tab:AIC_klu}).
...
\begin{equation}
E[n_{g,i}] = P(c_i | \DM, \DE, \phi) \times m_{a,i}
\end{equation} 
\begin{equation}
