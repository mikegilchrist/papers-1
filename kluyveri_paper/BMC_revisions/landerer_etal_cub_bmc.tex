...
\marginpar{This is too focused on our method.}
\end{abstract}
...
The signature of mutation bias is largely determined by both the organism's internal cellular environment, such as their DNA repair genes, and its external environment, such as UV exposure.
While mutation bias is an omnipresent evolutionary force, its impact on codon usage bias (CUB) can be countered or reinforced by natural selection. 
In contrast to mutation bias, the signature of selection on CUB is largely determined by the organism's cellular environment, such as its tRNA species, their copy number, expression level, and post-transcriptional modifications.
In general, the strength of selection on the CUB of an individual gene increases with its expression level \citep{gouy1982, ikemura1985, bulmer1990}, specifically its protein synthesis rate \citep{gilchrist2007}.
Thus as its protein synthesis increases, CUB of a gene shifts from a process dominated by mutation to a process dominated by selection.
The balance of mutation and selection on codon usage is also affected by genetic drift which, in turn, is an inverse function of the organism's effective population size $\Ne$.
Disentangling these evolution forces is of keen interest to biologists; \ROC allows us disentangle the contribution of mutation, selection, and drift to the patterns of CUB encoded across an species' genome \citep{gilchrist2007, ShahAndGilchrist2011, wallace2013, gilchrist2015}.
...
To illustrate these ideas, we analyze the CUB of the genome of the yeast \emph{Lachancea kluyveri}, which is sister to all other \emph{Lachancea spp.} \marginpar{mikeg:it is traditional to use \emph{Genus spp.} when discussing species within a genus.}
The Lachancea clade diverged from the Saccharomyces clade, prior to its whole genome duplication $\sim 100$ Mya ago \citep{MHM2015,Beimforde2014}. \marginpar{mikeg: Is the Lachancea clade synonymous with the \emph{Lachancea} genus? If so use \emph{Lachancea}}
Since that time, \kluyveri lineage has experienced a large introgression of exogenous genes (1 Mb, 457 genes) which is found in all of its populations \citep{payen2009,friedrich2015}, but in no other known Lachancea species \citep{vakirlis2016}.
...
While previous studies \marginpar{mikeg: cite these 'previous studies'.} have used information on gene expression to separate the effects of mutation and selection on codon usage, \ROC does not need such information but can provide it.
...
We use \ROC to separately describe the two cellular environments reflected in the \kluyveri genome; the signature of the current environment in the endogenous genes and the decaying signature of the exogenous environment in the introgressed genes.
Our results indicate that the current difference in \GC between endogenous and exogenous genes is mostly due to the differences in mutation bias of their ancestral environments.
...
These two sets where initially identified based on their striking difference in \GC \cite{payen2009}, with very little overlap in \GC between the two sets (Figure \ref{fig:gc_comp_phi}a).
