...
We utilize the \DM estimates for all two codon amino acids and infer the age of the introgression to be on the order of $6\pm1\times 10^8$ generations. 
...
\kluyveri experiences between one and eight generations per day, we therefore expect the introgression to have occurred about $205,000 \pm XXX$ to $1,600,000 \pm XXX$ years ago which is  longer than previous estimates of \citet{friedrich2015}.
...
Assuming that differences in mutation bias will decay more slowly than differences in selection bias, we predict that the \DM signal of the source cellular environment will have decayed to be within one percent of the \kluyveri environment within about $5\times 10^9 \pm XXX$ generations.
...
Choice of reference codon does reorganize codon families relative to each other, therefore all parameter estimates are relative to the mean for each codon family.
\begin{equation}
...
Our estimates of \DM can be directly related to the steady state of equation \ref{mut_ode}.
\begin{equation}
\frac{m_{2,1}}{\mu_{1,2} + \mu_{2,1}} = \frac{1}{1+\exp(\DM)}
\end{equation}
Solving for $\mu_{1,2}$ gives us $\mu_{1,2} = \DM\exp(\mu_{2,1})$ which allows us to rewrite and solve equation \ref{mut_ode} as
\begin{equation}
c_1(t) = \frac{\exp(-t(1+\DM)\mu_{2,1})\exp(t(1+\DM)\mu_{2,1}) + (1+\DM)K}{1+\DM}
\label{mut_close_form}
\end{equation}
where K is
\begin{equation}
K = \frac{-1 + c_1(0) + c_1(0)\DM}{1+\DM}
\end{equation}
...
Initial codon frequencies $c_1(0)$ for each codon family where taken from our estimates of \DM from \gossypii. 
...
\begin{equation}
E[n_{g,i}] = \frac{\exp(-\DM_i -\DE_i\phi_g)}{1+\sum_j^C \exp(-\DM_j -\DE_j\phi_g)}\times m_{a_i}
\end{equation} 
$m_{a_i}$ is the number of occurrences of amino acid $a$ that codon $i$ codes for.

We report the fitness burden of the introgression as $\Delta sN_e = (sN_e)_{I} - (sN_e)_{Endo}$ where $(sN_e)_{I}$ is either the fitness burden at the time of the introgression or presently.
