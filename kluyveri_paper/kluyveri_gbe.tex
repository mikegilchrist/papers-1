...
These introgressed exogenous genes replaced 497 genes, constituting the entire left arm of \LKlu's C chromosome, and were identified due to their $\sim 13\%$ elevation in \GC bias relative to the remaining, endogenous genes of the \Klu genome \citep{payen2009, friedrich2015}.

In order to study the effects of introgression and the resulting mismatches in codon usage in the \LKlu genome,  we use \ROC, a mechanistic model of codon usage bias evolution grounded in population genetics.
...
\ROC also allows us to predict a gene's average protein production rate based on its individual codon usage pattern with a precision comprable to that of more direct empirical methods \citep{gilchrist2015}.
By fitting \ROC separately to \LKlu's endogenous and exogenous sets of genes we generate a quantiative description of their signatures of mutation bias and natural selection for efficient protein translation.
Our results indicate that the difference in \GC between endogenous and exogenous genes mostly due to differences in mutation bias.
...
In order to identify a potential source lineage for \LKlu's exogenous gene set we fit \ROC to the genomes of 38 yeast species.
...
We also compared the synteny of \LKlu's exogenous genes to a subset of these lineages.
