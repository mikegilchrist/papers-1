...
In order to quantiatively describe \LKlu's codon usage environment, we fitted a bayesian, mechanistic model of codon usage bias evolution, ROC-SEMPPR, to \Klu's endogenous gene in order to estimate the strength of mutation bias and selection on codon usage.
We then compared these parameter estimates to those we obtained by fitting ROC-SEMPPR to \LKlu's exogenous genes, which provides a biased estimate of the ancestral environment of the exogenous genes.
Our results indicate the differences in codon usage between \LKlu's endogenous and exogenous genes are largely due to differences in mutation bias, rather than selection.
...
More specifically, we utilize the Ribosome Overhead Cost (ROC) version of \citet{ShahAndGilchrist2011} of the more general Stochastic Evolutionary Model of Protein Production Rates (SEMPPR) introduced in \citet{Gilchrist2007} using the R software package \text{AnaCoDa}\citet{landerer2018}.   
...
% between \kluyveri and the donor lineage of the exogenous genes.
...
%      For example, if the relationship between expression and fitness is flat, then CU cost could be reduced if expression is reduced. -- This specific example would seem to fall into the regulatory realm below since we'd need a reduction in expression simultaneously.
...
\bibliography{kluyveri_paper,mike}
