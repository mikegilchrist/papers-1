...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption{Genetic load $\Delta s$ (left) at the time of introgression ($\kappa = 5$), and (right) currently ($\kappa = 1$).}
...
As \DE is defined as $\DE = 2N_eq(\eta_i-\eta_j)$, we can not distinguish if $\kappa$ is scaling protein synthesis rate $\phi$, effective population size $N_e$, or the value of an ATP $q$ \citep{gilchrist2015}.
...
\begin{figure}[h]
    ...
    \begin{subfigure}
        \centering
        \includegraphics[width=.45\textwidth]{ch3/synteny_coverage.pdf}
    \end{subfigure}
    \begin{subfigure}
        \centering
        \includegraphics[width=.45\textwidth]{ch3/synteny_blocks_and_gc.pdf}
    \end{subfigure}
    \caption{Synteny relationship of the exogenous chromosome arm C-left with 13 yeast species. The size of the circle indicates the amount of synteny in units of standard deviation. 
    \gossypii displays the highest amount of synteny. The synteny relationship between the exogenous chromosome arm and \gossypii and the \GC of the exogenous chromosome arm is displayed on the right.}
...
\begin{figure}[H]
     \centering
	\includegraphics[width=\textwidth]{ch3/adaptation_total.pdf}
...
\begin{figure}[H]
	...
	\caption{Predictions of codon frequencies as a function of protein synthesis rate $\phi$ for all amino acids. 
	Solid, dashed, and dotted lines represent model fits for the endogenous gene set, the exogenous gene set, and the whole \kluyveri genome respectively.
	The bottom right panel shows corresponding histograms of the protein synthesis rate $\phi$.}
