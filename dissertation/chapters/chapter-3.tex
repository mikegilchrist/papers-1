...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Fitness burden $s = \DE \phi$ (a) at the time of introgression ($\kappa = 5$), and (b) currently ($\kappa = 1$). }
...
Even though the exogenous genes make up only $\sim10\%$ of the \kluyveri genome, when we fail to recognize these differences our estimates of \DM and \DE deviate substantial from their actual values (Figure XX).
...
This behavior is due, in part, to constraining $E[\phi] = 1$ which allows us to compare the efficacy of selection $s \Ne$  across genomes.
...
In the  first scenario, where the introgression fixed in spite of strong selection against it, it is worth noting that \kluyveri diverged from the rest of the Lachancea clade around 85 Mya [CITATION].
...
Assuming an effective population size $N_e$ on the order of $10^8$ [CITATION] and rapid fixation of the introgression, implies $10^{18}$ to $10^{19}$ opportunities for such an event to have occurred.

In the second scenario, where the introgression contained highly advantagous loci, one may wonder why recombination events did not limit the introgression to only the adaptive loci.
