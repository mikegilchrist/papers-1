...
Here we assess and compare the fit and adequacy of phylogenetic inferences made  wiht \phydms, using supplementary information on selection, and with \selac, a novel codon model of stabilizing selection on amino acids.
...
While commonly used models are therefore fast and easy to use, they lack biological realism.
...
These 263 discrete parameters are only $\sim5\%$ of the $19\times N = 4997$ parameters necessary to fully describe the site specific selection like it is done in \hb.
...
\begin{table}
  \centering
  \caption{Comparison of the best performing models by category based on their AIC values, where  $\log(Lik)$ is the log-likelihood of each model and $n$ is the number of model parameters estimated from the aligned sequence data.
...
More specifically, we simulated sequences forward in time from the ancestral state under the \phydms, and \selac selection to establish a point of reference and further assess model adequacy.

\begin{figure}
    \centering
    \begin{subfigure}
        \centering
        (a)\includegraphics[width=.45\textwidth]{ch4/simulated_gl_time_DMS_ancest.pdf}
    \end{subfigure}
    \begin{subfigure}
        \centering
        (b)\includegraphics[width=.45\textwidth]{ch4/simulated_gl_time_SELAC_ancest.pdf}
    \end{subfigure}
...
Simulations under the \phydms selection parameters show that the genetic load of 0.065 of the observed sequences is larger than the genetic load of the simulated sequences.
...
Simulations under the \selac inferred selection show that the genetic load of $2.4\times 10^{-7}$ of the observed sequences is less than the genetic load of the simulated sequences.
...
\begin{table}
  \centering
  \caption{Average genetic load at variant and invariant sites in the TEM alignment according to \selac and DMS}
  \begin{tabular}{lccc}
    \hline
...
We further assess the model adequacy of \phydms and \selac which we primarily define as the genetic load of the sequence of optimal amino acids according to the DMS and \selac selection parameters.
However, we also compare the sequence similarity of the sequence of optimal amino acids to the observed consensus sequence.
...
It would also allow for frequency dependent selection like \gy.
...
Experimentally inferred selection parameter for TEM were taken from \citet{stiffler2016}.
...
We report each model's log likelihood ($\log(Lik)$), and AIC.
...
Each sequence was simulated 10 times and we report average genetic load and sequence similarity as well as standard error.
...
To calculate an individual $\eta$ for each site, it is necessary to estimate the site sensitivity term $G$ for each site.
\selac, however, assumes $G$ is a gamma distributed random effect.
We estimated $G$ for each site using the \selac fit and the aligned sequences using a maximum likelihood approach.
As $G$ tends to infinity for invariant sites, we bound $G$ to a maximum of 300 to ensure numerical stability of followup calculations.
...
\begin{equation}
GL_i = \frac{w_{max,i} - \overline{w_i}}{w_{max,i}}
\end{equation}
