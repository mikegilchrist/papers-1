...
\subsection{\selac Shows Subtantially Greater Model Adequacy Than \phydms}
...
Simulations under the \phydms show that the genetic load of 0.065 of the observed sequences is larger than the genetic load of the simulated sequences.
...
Simulations under the \selac inferred selection show that the average genetic load of $2.4\times 10^{-7}$ of the observed sequences is substantially less than the genetic load of the simulated sequences ($1.3 \times 10^{-5}$ Figure \ref{fig:gl_sim}b).
...
\marginpar{Can you really defend this last sentence?
  It seems like \selac is off by 2 orders of magnitude while \phydms is within 1.
  Perhaps this is where you want to invoke pop. gen. principles.
}
Nevertheless, the \selac fit shows high model adequacy and predicts a zero genetic load at invariant sites.
...
\begin{table}
  ...
  \begin{tabular}{lccc}
    ...
    Category 		& \# Residues	& \multicolumn{1}{c}{\selac} & \multicolumn{1}{c}{DMS} \\ \hline 
...
\marginpar{Where are these values reported?}
...
As the maximum likeihood value of $G$ tends to infinity for invariant sites, we bound $G$ to a maximum of 300 to ensure numerical stability of followup calculations.
\marginpar{How does this upper bound affect your calculations?
  It has no effect b/c these are invariant sites, but you need to explain this.
  }
...
\singlespacing
\marginpar{Why did you add G4 to the selac model here but not elsewhere?
  Does this need to be mentioned in text?}
\begin{longtable}{clrrrrrr}
  \caption{Maximum likelihood fits of 229 models of nucleotide and codon evolution ranked by \AIC values.
  See \citet{IQTree} for model definitions.}
  \label{tab:AIC_full}
  \\ 
  \toprule
