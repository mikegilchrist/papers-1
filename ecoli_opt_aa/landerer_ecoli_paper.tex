...
noindent{\Large \bf Site specific, \PC based phylogenetic models outperform experimentally informed models and avoid laboratory bias}
...
To remedy this issue, two routes have been taken, the devoplement of more realistic models which infer site specific selection in a restricted manner and the use of supplementary information on site specific selection.
...
\marginpar{do you need to be so general in terms of your 'two routes'?
  Why not talk about DMS and \phydms and \selac first and then make a more general statement at the end of the abstract?}
\newpage
...
The \phydms model, parameterized by site specific selection parameters from \citet{stiffler2016} performs second best and provides an improvement of 289 AIC units for the best uniform site model \emph{SYM}+R2 (SYM) \citet{zharkikh1994}. 
...
\marginpar{While the statement about the \gy in the original form is true, it is misleading if this information is not used in IQTree.
  Please confirm that you looked at the IQTree code and verified they use the \PC version of GY.
  Their description in the documentation suggests otherwise.
  }


%\phydms does not infer any site specific parameters from the sequence data, but utilizes site specific selection parameters from DMS experiments.
...
\begin{table}
  ...
  \caption{Comparison of the best performing models by category based on their AIC values, where  $\log(\Lik)$ is the log-likelihood each model and $n$ is the number of model parameters estimated from the aligned sequence data.
    ...
    The best performing nucleotide model is the XXXX variant of \citet{zharkikh1994}'s symmetrical model \emph{SYM+R2}. 
    The best performing codon model is YYYY variant of \gy    
  See Table \ref{tab:AIC_full} for results from all ZZZ models tested.
  \textbf{NOTE: Need to fully define models!  What does R2 mean?  F1X4?
  Apparently my previous request was unclear, but you should use a consistent set of numbers when talking about the number of models compared.}
}  
...
In addition, polytomies are widespread in all estimated phylogenies, and generally, the shortest branch length are estimated by SYM.
...
\marginpar{You need some kind of intro on your metrics of model adequacy.
  I've tried to help by moving some of the text towards the end up to here.
  Also, the subsection title is too long.
  Finally, I think for the paper we submit, it will be simpler to focus on the differences in sequence similarity and move most of the GL stuff to SM.}
...
\marginpar{DMS or \phydms?
  I can see why using DMS makes sense, but you previously focused on \selac vs. \phydms.
  As a result, you need to either explain to the reader what's going on orjust keep it simple and talk about the selection parameters used in \phydms (which I would recommend).
  You can mention that these parameters are taken from the DMS experiments if you feel that is necessary.
  }
...
\marginpar{Please ensure that the new ordering of the info at the end of this paragraph and the ones that follows makes sense.
I suspect the above results can be subsumed into the next paragraph.}
...
Again we find a significant difference in genetic load between the half for which the \selac and DMS predictions of the optimal amino acid agree and the half for which they differ of $0.004$ and $0.0158$, respectively (paired t-test, $p = 2\times 10^{-5}$).
...
\marginpar{Why are you unnecessarily opening this can of worms?
  Do these calculations advance your argument in any substantive manner?
  }
...
\marginpar{$\Lik$ not defined and maximizing \Lik for model fitting not mentioned!}
...
\marginpar{The genetic load $L$ and the likelihood $L$ are indistinguishable.
  Why not use $\text{Lik}$ for the latter?}
%We, however, rescaled all fitness estimates such that $w_{max,i} = 1$. \marginpar{This is unnecessary.}
