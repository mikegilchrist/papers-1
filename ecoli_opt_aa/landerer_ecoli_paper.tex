...
% Insert your title:
\noindent{\Large \bf Experimentally informed phylogenetic models are biased towards laboratory conditions and can be improved upon by mechanistic models of stabilizing selection.}\marginpar{My attempt: Site specific, \PC based phylogenetic models outperform and lack laboratory bias of experimentally informed models.}
\bigskip
...
Most popular, the codon model by \citet{GoldmanAndYang1994} (\gy) and its derivatives.\marginpar{this is not a sentence.} 
However, \gy is commonly misinterpreted and provides only a restricted selection scenario that is best described as frequency dependent selection \citep{beaulieu2019}.\marginpar{Should cite Hughes and other papers cited in \citet{beaulieu2019}.}
...
\marginpar{Sentence moved to previous paragraph and needs to be revised.
E.g., ``One common property of all of these models is the fact that they use a single substitution model across all sites. As a result, every site, whether a nucleotide,  amino acid, or codon, have the same equilibrium distribution.''
Would be good to introduce terminology to describe these non-site specific models.
Uniform site models?
...
\marginpar{Removed HB b/c cited next.  
Also, aren't there earlier papers that provide evidence for site heterogeneity?
}
...
\marginpar{Using `selection parameters' instead of `fitness estimates', `fitness parameters', `fitness values', ...  here and throughout.}
This need for estimating a large number of selection parameters from the phylogenetic data makes the application of \citet{HalpernAndBruno1998} model unfeasible for most studies.
...
Bloom and others \citep{bloom2014, bloom2017, hilton2017} report that using DMS fitness estimates ...
\marginpar{need to complete sentence.}
...
\marginpar{Why is 'over many generations' relevant or meaningful? Are you referring to the idea that these parameter estimates are precise? (but inaccurate given the inter-lab variation and lab vs. wild variation?}
...
A further limitation of DMS data stems from the fact that laboratory conditions generally differ from those organisms usually encounter in the wild.
More troubling is the fact that variation between DMS experiments can lead to significant differences betweeen model fits \citep{hilton2017}.
\marginpar{order of previous two sentences could be switched and then revised.
Lab variation is substantial.
Lab to wild variation is likely even larger.}
DMS selection parameters depend on factors like the initial library of mutations which, if extensive,leads to a heterogeneous population of competing individuals unlikely to be observed in the wild.
\marginpar{you seem to be conflating the effects of a specific set of initial conditions (ICs) and the effects of high genetic heterogeneity together.
Is the IC effect the fact that the initial frequencies differ from those assumed or  just the heterogeneity?
If it's the former you're concerned with then it would seem like these are separate effects (but share an interaction term).
If it's the latter, then I don't think it's worth adding this point about ICs.}
...
For example, Lartillot and colleagues mitigate the high numbers of  parameters required by \hb's codon model  using a  site categorization approach where a limited, but \emph{a priori} unspecified, number of site categories are estimated from the phylogenetic data \citep{LartillotAndPhilippe2004,le2008}.
\marginpar{
There are other more recent work in from this group that should be cited!
See the \selac paper.
Is `phylogenetic data' an okay term?
Would 'sequence data' or 'aligned data' be better?}
More recently,  \citep{beaulieu2019} introduced a new codon model, \selac,  where $20$ site categories are assumed \emph{a priori} to underly the \hb model .
\selac combines \PC properties and site specific heterogeneity in the strength of selection together using a simplistic nested modelling approach.
...
In order to assess and compare ... 
\marginpar{Clearly state what you're doing, comparing the fit and adequacy of phylogenetic inferences made using DMS and \selac.} 
Following \citep{hilton2017}, we used the DMS based selection estimates from  \citet{stiffler2016} for $\beta$-lactamase TEM.
We use \phydms \citep{hilton2017} to fit 49 TEM sequences observed in natural populations of \emph{E.~coli} from \citet{bloom2017} using DMS inferred site specific selection on amino acids.
\marginpar{Check that citations are correct in previous two sentences.
Also, revise them so its clear there are two types of data: natural sequence data used for the phylogenetics and DMS based selection parameters.}
...
We attribute \phydms's poor model adequacy to ... 
\marginpar{complete sentence}.
...
We compared \selac and \phydms to 131 nucleotide and 97 codon uniform site models fitted using IQTree \citep[][see Table \ref{tab:AIC_selac} for the best performing models and Table \ref{tab:AIC_full} for all models]{nguyen2015}.
\marginpar{were additional variants also  fitted?   I thought the total  models fitted, including variants was 97+131=228.
As a result, I cut the 'and their variations'}
...
This better performance is in spite of the fact that \selac required the specification of one optimal amino acid for each of the 272 codons in our TEM alignment.
...
\marginpar{
This caption needs help. 
I've already corrected 2 minor errors, but as it stands the last two models are not adequately defined and no mention of conservative nature of our estimate of $n$.
In addition, your use of 'stabilizing' selection is the first one since the title.
Either include this term earlier (and discuss) or cut from here.
} 
}  
...
However, \gy is outperformed by XXXX  nucleotide and codon models like \emph{SYM}+R2.
...
\marginpar{Does IQTree's implementation of \gy really use \PC? 
Most references to \gy are to a derivative of it that ignores these properties.
Recall our previous discussions about this in the \selac paper.
Also, cite Hughes here, not just beaulieu.
}
...
If we utilize the \phydms inferred topology, with \selac, we find a very similar nut slightly worse model fit to our original \selac fit ($\Delta AIC = 2$).
This indicates that the topology has less impact on the model fit as the assumed optimal amino acid sequence, likely due to the short branches and many polytomies in the phylogeny.
...
We define model adequacy as similarity of the sequence of selectively favored, or optimal amino acids with the observed consensus sequence.
...
Together this suggests that DMS selection parameters are not very informative about selection in the wild.

In contrast, the \selac fit shows high model adequacy.
...
This may not be to surprising given that \selac only uses the phylogenetic data and no experimental supplementary data.
...
\selac predicts a zero genetic load at invariant sites, while the genetic load predicted at these sites by DMS is not significantly different from the genetic load at the variant sites (Table \ref{tab:selection}).
...
In total, we find 100 sites for which \selac estimates a genetic load of zero but the DMS estimates predict a non-zero genetic load.
All 100 cases show a significant difference in the likelihood between the \selac and the DMS inferred optimal amino acid given the observed phylogenetic data.
...
We also assess the genetic load of the observed sequences according to the DMS and \selac estimates fitness landscape.
...
Given \ecoli's rapid doubling time of 15 hours in the wild \citep{gibson2018}, this means that an average mutation should sweep through the population in 1.5 years.

In contrast, estimates of selection obtained by \selac show the observed sequences near a fitness peak.
This is consistent with theoretical population genetics results derived above.
...
These laboratory conditions can lead to the bias in selection parameters we observe in this study.
...
\selac on the other hand, better explains the evolution of TEM in the wild and does not require the supplementary selection data but provides such estimates from the phylogenetic data.
...
This may be appropriate for closely related organisms in similar environment but not when deeper divergence between distantly related species is to be studied.
...
In particular, a hidden markov approach would allow for frequency dependent selection like it is modeled by \gy.
This may be appropriate for TEM as it plays a role in chemical warfare between microorganisms.
...
We followed \citep{bloom2017} to convert the experimental fitness parameters into site specific equilibrium frequencies for \phydms. 
