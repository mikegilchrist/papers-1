...
However, \gy is commonly misinterpreted and provides only a restricted selection scenario that is best described as frequency dependent selection \citep{beaulieu2019}.
...
This large number of parameters which have to be estimated from the phylogenetic data makes the application of this model often unfeasible.

To overcome the issue of estimating an unfeasible amount of parameters from phylogenetic data, it was proposed to use supplement the inference with data from deep mutation scanning experiments (DMS).
...
Using such supplementary site specific selection estimates allows for the fitting of complex site specific models to smaller phylogenetic data sets, and it improves model fit \citep{bloom2014, bloom2017, hilton2017}.

However, such DMS selection estimates are not always available.
...
This, however, limits the application of DMS selection estimates for phylogenetic inference greatly; to organisms with short generation times and ones that can be manipulated under laboratory conditions.
Laboratory conditions also greatly differ from the condition an organism experiences in the wild, therefore, it is questionable how well selection estimates inferred under such conditions reflect selection in the wild and are applicable for phylogenetic inference.
DMS selection estimates depend on factors like the initial library of mutations which - if extensive - leads to a heterogeneous population of competing individuals unlikely to be observed in the wild.
In addition, \citet{hilton2017} showed variation between DMS experiments can lead to significant differences in model fit.

Alternatively, better models could be be developed instead of using questionable supplementary data for phylogenetic inference.
...
\selac is limited to $20$ site categories, one for each possible optimal amino acid, but with a clear biological interpretation of each site category.
\selac incorporates \PC properties in a simplistic nested model and is rooted in population genetics.
...
Our results show that site specific selection inferred by DMS improve model fit which is consistent with previous work \citep{bloom2014,bloom2017}, but supplementing phylogenetic inference with unnatural data causes poor model adequacy. 
...
Further, we find evidence that the DMS inference of selection on amino acids is inconsistent with selection in the wild at many sites, leading to poor model adequacy.
...
We compare \selac and \phydms to 131 nucleotide and 97 codon models and their variations (see Table \ref{tab:AIC_full} for all models).
\selac shows the best model fit and provides an improvement of 582 AIC units over the empirically informed model fit by \phydms despite the 272 more model parameters estimated from the phylogenetic data (Table \ref{tab:AIC_selac}). 
\phydms, parameterized by site specific selection estimates from \citet{stiffler2016} performs second best and provides an improvement of 289 AIC units for the best non-site specific model, the symmetric nucleotide model by \citet{zharkikh1994}. 
...
However, \gy is outperformed by various nucleotide models like \emph{SYM}+R2.
...
For example, at site 157, we observe the non-polar, hydrophobic amino acids methionine, but the DMS experiment predicts that the polar, hydrophilic amino acid threonine provides 
the highest fitness.
...
Together this suggests that DMS selection estimates are not very informative about selection in the wild and that \selac is a more appropriate tool to obtain such estimates.
...
This is consistent with theoretical population genetics (see Discussion).
...
\begin{longtable}{clrrrrrr}
  ...
  \\ 
  \toprule
  No. & Model & $\log(\Lik)$ & n & AIC & $\Delta$AIC  \\   \hline \endfirsthead
  \caption*{Table \ref{tab:AIC_full} Continued}\\\toprule
  No. & Model & $\log(\Lik)$ & n & AIC & $\Delta$AIC  \\   \hline \endhead
  \hline \endfoot
  \bottomrule
  \endlastfoot
